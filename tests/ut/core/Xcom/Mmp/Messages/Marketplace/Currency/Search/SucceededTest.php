...
class Xcom_Mmp_Message_Marketplace_Currency_Search_SucceededTest extends Xcom_TestCase
{
    /**
     * @var Xcom_Mmp_Model_Message_Marketplace_Currency_Search_Succeeded
     */
    protected $_object     = null;

    protected $_messageBody    = array(
        'currencies'        => array('USD', 'UAH'),
        'marketplace'       => 'test',
        'siteCode'          => 'US'
    );
    ...
    {
        parent::setUp();
        $this->_object = Mage::helper('xcom_xfabric')->getMessage('marketplace/currency/searchSucceeded');
    }
    ...
    {
        $this->assertInstanceOf('Xcom_Mmp_Model_Message_Marketplace_Currency_Search_Succeeded', $this->_object);
    }

    public function testGetTopic()
    {
        $topic = $this->_object->getTopic();
        $this->assertEquals('marketplace/currency/searchSucceeded', $topic);
    }
    ...
    {
        $objectMock = $this->mockModel('xcom_mmp/message_marketplace_currency_search_succeeded',
                                       array('_saveCurrency'));

        $objectMock->expects($this->any())
            ->method('_saveCurrency')
     ...
     * for xcom_mmp/currency resource model
     *
     * @return void
     */
    public function testSavePaymentMethod()
    {
        ...
        $resourceData           = $this->_messageBody['currencies'];
        $resourceMock   = $this->mockResource('xcom_mmp/currency', array('upgrade'));
    ...
    public function testGetResponseData()
    {
        ...
        $expectedResponse   = $this->_messageBody['currencies'];
    ...
    public function providerValidateSchema()
    {
        return array(
            array(
                array_diff_assoc($this->_messageBody, array('currencies' => $this->_messageBody['currencies']))
            ),
            array(
