...
if (defined('COMPILER_INCLUDE_PATH')) {
     ...
     * Set include path
     */
    $paths[] = TBP . DS . 'framework';
    $paths[] = BP . DS . 'app' . DS . 'code' . DS . 'local';
    $paths[] = BP . DS . 'app' . DS . 'code' . DS . 'community';
    $paths[] = BP . DS . 'app' . DS . 'code' . DS . 'core';
    $paths[] = BP . DS . 'tests' . DS . 'integration' . DS . 'modules';
    $paths[] = BP . DS . 'lib';
...
require_once $rootDir . DIRECTORY_SEPARATOR . 'tests/framework/Config.php';
...
final class Mage
{
     ...
     * @var bool
     */
    static private $_useMockConfig = false;

    /**
     * Mocked config property
     * @var Mage_Core_Model_Config
     */
    static private $_mockConfig;
    ...
    public static function getVersion()
    {
        ...
        return trim("{$i['major']}.{$i['minor']}.{$i['revision']}"
        . ($i['patch'] != '' ? ".{$i['patch']}" : "") . "-{$i['stability']}{$i['number']}", '.-');
    }
    ...
    {
        $mock = self::registry('store_config');
        if ($mock && isset($mock[$path])) {
            return $mock[$path];
        }
    ...
    {
        if(self::$_useMockConfig){
            return self::$_mockConfig;
        }
        return self::$_config;
    }

    public static function setConfigMock($mockConfigModel)
    {
        self::$_useMockConfig = true;
        self::$_mockConfig = $mockConfigModel;
    }

    public static function setUseMockConfig($value)
    {
        self::$_useMockConfig = (boolean)$value;
    }
    ...
    public static function dispatchEvent($name, array $data = array())
    {
        ...
        self::unregister('test_framework_dispatched_event');
        self::register('test_framework_dispatched_event', $events);

        if (!self::$_allowDispatchEvent) {
            return;
        }
    ...
    {
        $mockModels = self::registry('mock_models');
        if ($mockModels && isset($mockModels[$modelClass])) {
            return $mockModels[$modelClass];
        }
    ...
    {
        $mockModels = self::registry('mock_resource_models');
        if ($mockModels && isset($mockModels[$modelClass])) {
            return $mockModels[$modelClass];
        }
...
//        echo $registryKey; die();
    ...
    {
        $mockModels = self::registry('mock_helpers');
        if ($mockModels && isset($mockModels[$name])) {
    ...
    public static function app($code = '', $type = 'store', $options = array())
    {
        if (null === self::$_app) {
            ...
            self::$_config = new Xcom_Test_Config($options);
    ...
    private static $_allowDispatchEvent = false;

    public static function registerMockHelper($name, $object)
    {
        $mockModels = self::registry('mock_helpers');
        if (!$mockModels) {
            $mockModels = array();
        }
        $mockModels[$name] = $object;
        self::unregister('mock_helpers');
        self::register('mock_helpers', $mockModels);
    }

    public static function registerMockModel($name, $object) {
        $mockModels = self::registry('mock_models');
        if (!$mockModels) {
            $mockModels = array();
        }
        $mockModels[$name] = $object;
        self::unregister('mock_models');
        self::register('mock_models', $mockModels);
    }


    public static function registerMockResourceModel($name, $object) {
        $mockModels = self::registry('mock_resource_models');
        if (!$mockModels)
            $mockModels = array();
        $mockModels[$name] = $object;
        self::unregister('mock_resource_models');
        self::register('mock_resource_models', $mockModels);
    }

    public static function registerStoreConfigMock($name, $value)
    {
        $conf = self::registry('store_config');
        if (!$conf)
            $conf = array();
        $conf[$name] = $value;
        self::unregister('store_config');
        self::register('store_config', $conf);
    }
    ...
    public static function unregisterMockHelper($name)
    {
        self::unregister('_helper/' . $name);
    }

    public static function registerModelSingleton($name, $object)
    {
        self::register('_singleton/' . $name, $object);
    }

    public static function setAllowDispatchEvent($flag)
    {
        self::$_allowDispatchEvent = (bool)$flag;
    }
}
