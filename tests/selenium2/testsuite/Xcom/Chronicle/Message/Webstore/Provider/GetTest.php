...
class Xcom_Chronicle_Model_Message_Webstore_Provider_Get_Outbound extends Xcom_Xfabric_Model_Message_Request
{
 ...
 * @license     http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */
class Xcom_Chronicle_Message_Webstore_Provider_GetTest extends Mage_Selenium_TestCase
{
     ...
     *  @test
     */
    public function testGetWebstoreProvider()
    {
        /* issue the search */
        $this->_postSearch(
        ...
        $this->_findGetWebstoreProviderMessage($latestMessageId);
    }


    protected function _findGetWebstoreProviderMessage($startMessageId)
    {
        $found = false;
        ...
        while (!$found && $retries < 10) {
            ...
            for ($c = 0; $msgId <= $latestMsgId && !$found; $c++, $msgId++) {
                $found = $this->_verifyGetWebstoreProviderMessage($receivedMsgs[$c]);
            }
    ...
    protected function _verifyGetWebstoreProviderMessage($msg)
    {
        $topic = $msg->{'topic'};
        Mage::log("Topic: " . print_r($topic, true), null, 'debug.log', true);

        if (strcmp($topic, '/com.x.webstore.v1/WebStoreMetadataProvision/GetAllWebStoreProviderSucceeded') == 0) {
            return $this->_verifyGetWebstoreSuccessMessage($msg);
        }
        else{
            return false;
        }
    ...
    protected function _verifyGetWebstoreSuccessMessage($msg) {
        $providerName = $msg->{'message'}->{'providerName'};
    ...
    {
        $c = curl_init($fabricURL . "/com.x.webstore.v1/WebStoreMetadataProvision/GetAllWebStoreProvider");
        ...
        curl_setopt($c, CURLOPT_HTTPHEADER, array(
            ...
            "X-XC-SCHEMA-URI: https://api.x.com/ocl/com.x.webstore.v1/WebStoreMetadataProvision/GetAllWebStoreProvider/1.0.0",
            "Content-Type: " . $contentType
        ));

        $body = new Xcom_Chronicle_Model_Message_Webstore_Provider_Get_Inbound();
