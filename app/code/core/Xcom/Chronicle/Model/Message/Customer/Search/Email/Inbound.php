...
class Xcom_Chronicle_Model_Message_Customer_Search_Email_Inbound extends Xcom_Xfabric_Model_Message_Response
{
    /**
    ...
    {
        $this->_topic               = 'com.x.customer.v1/CustomerSearch.Email/SearchCustomer';
    ...
    public function process()
    {
        ...
        try {
            if ($this->_validateSchema()) {
                ...
                Mage::helper('xcom_xfabric')->send('com.x.customer.v1/CustomerSearch.Email/SearchEmailCustomerSucceeded', $response);
            }
        }
            ...
            Mage::helper('xcom_xfabric')->send('com.x.customer.v1/CustomerSearch.Email/SearchCustomerFailed',$errorResponse);
        }
        catch(Exception $ex){
            if(!is_null($ex)){
    ...
    protected function _processSearchQuery(&$data)
    {
        ...
        if ($offset < $customers->getSize()) {
            ...
            foreach ($customers as $customer) {
                ...
                $results[] = Mage::getModel('xcom_chronicle/message_customer_email', $c)->toArray();
            }
        }
        ...
        if ($shouldUseOrders) {
            ...
            foreach ($orders as $order) {
                $results[] = Mage::getModel('xcom_chronicle/message_customer_guest_email', $order)->toArray();
            }
        }
